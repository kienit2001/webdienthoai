...
public interface ProductRepository extends JpaRepository<Product, Integer> {
    ...
    public Page<Product> searchFilterProduct(double minPrice, double maxPrice, String brandId, Pageable pageable);

    @Query("SELECT p FROM Product p WHERE (p.name LIKE %:keyword% OR p.shortDescription LIKE %:keyword%) AND " +
            "(p.price between :minPrice AND :maxPrice) AND p.isActive <> FALSE " +
            "AND ((:brandId = 'all' AND p.brands.id is not null ) OR (:brandId <> 'all' AND p.brands.id = :brandId))")
    public Page<Product> searchWithKeywordFilterProduct(String keyword, double minPrice, double maxPrice, String brandId, Pageable pageable);
